...
function renderTransactions() {
    ...
    transactionTableBody.innerHTML = filtered.map(t => `
        <tr class="border-b dark:border-[#cccccc]">
            <td class="text-left py-2">${t.date}</td>
            <td class="text-left py-2">${t.description}</td>
            <td class="text-left py-2 capitalize">${t.category}</td>
            <td class="text-left py-2 ${t.type === "income" ? "text-green-700 dark:text-green-500" : "text-red-700 dark:text-red-500"}">
                ${t.type === "income" ? "+" : "-"}${formatAmount(t.amount)}
            </td>
            <td class="text-left py-2"><button class="edit-btn cursor-pointer" data-id="${t.id}">Edit</button></td>
            <td class="text-left py-2"><button class="delete-btn cursor-pointer" data-id="${t.id}">Delete</button></td>
        </tr>
    `).join("")

    const thead = document.getElementById("transaction-thead")
    thead.classList.toggle("hidden", filtered.length === 0)
...
function updateChart() {
  ...
  cashChart.data.datasets[0].data = sorted.map((t) =>
    t.type === "income" ? t.amount : 0,
  );
  cashChart.data.datasets[1].data = sorted.map((t) =>
...
keyEnter(
  [typeInput, descriptionInput, amountInput, dateInput, categoryInput],
